...
- Sorting.. for example ```select * from users where lastName like 'A%' order by createdOn desc;``` - This is nearly completed but the sort method isn't sorting the specialized ``[]interface{}`` which needs to be looked at further. 
