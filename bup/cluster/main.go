...
func (cursus *Cursus) QueryNode(n *NodeConnection, body []byte) string {
	...
	return line
unavailable:
	return fmt.Sprintf(`{"statusCode": 105, "message": "Node %s unavailable."}`, n.Conn.RemoteAddr().String())
}
...
func (cursus *Cursus) QueryNodes(connection *Connection, body map[string]interface{}) error {
	...
	for _, n := range cursus.NodeConnections {
		responses[n.Conn.RemoteAddr().String()] = cursus.QueryNode(n, jsonString)
	}
...
func main() {
	...
	for i := 0; i < cursus.Config.ConnectionQueueWorkers; i++ {
		cursus.Wg.Add(1)
		go cursus.ConnectionEventLoop(i)
	}
